...
const findSchool = () => {
    ...
    openWin = window.open('/staff/findschool', 'childForm', 'width=570, height=350, resizable=no, scrollbars=yes');
};
...
const setMealData = async(year, month) => {
	$.ajax({
		url: `${location.pathname}/lunch_meals?year=${year}&month=${month}`,
		method: 'get',
		dataType: 'json',
		async: true,
		success: (result) => {
...
const setLunchMealLayerData = async(YMD) => {
	$.ajax({
		url: `${location.pathname}lunch_meal?YMD=${YMD}`,
		method: 'get',
		dataType: 'json',
		async: true,
		success: (result) => {
            ...
            switch(data.mealServiceDietInfo[0].head[1].RESULT.CODE){
                case 'INFO-000':
                    meal = data.mealServiceDietInfo[1].row[0];
                    document.getElementById('meal_type').innerText = meal.MMEAL_SC_NM;
                    document.getElementById('meal_ymd').innerText = setAPIDateFormat(meal.MLSV_YMD);
                    document.getElementById('meal_diet').innerText = meal.DDISH_NM.replace(/<br\/>/ig, '\n');
                    document.getElementById('meal_cal').innerText = meal.CAL_INFO;
                    document.getElementById('meal_origin').innerText = meal.ORPLC_INFO.replace(/<br\/>/ig, '\t');;
                    document.getElementById('meal_ntr').innerText = meal.NTR_INFO.replace(/<br\/>/ig, '\t');;
                    break;
            };
		},
		error: (jqXHR) => {
...
const setScheduleData = async(year, month) => {
	$.ajax({
		url: `${location.pathname}schedules?year=${year}&month=${month}`,
		method: 'get',
		dataType: 'json',
		async: true,
		success: (result) => {
...
const setScheduleLayerData = async(YMD) => {
	$.ajax({
		url: `${location.pathname}schedule?YMD=${YMD}`,
		method: 'get',
		dataType: 'json',
		async: true,
		success: (result) => {
	        ...
	        switch(data.SchoolSchedule[0].head[1].RESULT.CODE){
	            case 'INFO-000':
	                ...
	                document.getElementById('schedule_ymd').innerText = setAPIDateFormat(ymd);
...
const setMyScheduleData = async(year, month) => {
	$.ajax({
		url: `${location.pathname}my_schedules?year=${year}&month=${month}`,
		method: 'get',
		dataType: 'json',
		async: true,
		success: (result) => {
			result.data.map((my_schedule) => {
	            let start_date = new Date(my_schedule.schedule.startDate);
	            let end_date = new Date(my_schedule.schedule.endDate);
	            for(let i = 0; i < Math.abs(end_date - start_date) / (1000 * 3600 * 24) + 1; i++){
	                ...
	                if(schedule_id_span.parentNode.childElementCount < 4){
	                    let calHtml = "";
	                    calHtml += `<div>
	                    <a class='my_schedule' href='#my_schedule_layer' style='font-size: 13px; font-weight: bolder; color: #515151; text-decoration: none; margin-top: 10px;' onclick="setMyScheduleLayerData(${my_schedule.schedule.id})">
...
const setMyScheduleLayerData = async(schedule_id) => {
	$.ajax({
		url: `${location.pathname}my_schedule_by_id?id=${schedule_id}`,
		method: 'get',
		dataType: 'json',
		async: true,
		success: (result) => {
	        ...
	        document.getElementById('my_schedule_subject').innerText = data.schedule.subject;
	        document.getElementById('my_schedule_content').innerText = data.schedule.content;
	        document.getElementById('my_schedule_start_date').innerText = setDateFormat(data.schedule.startDate);
	        document.getElementById('my_schedule_end_date').innerText = setDateFormat(data.schedule.endDate);
		},
		error: (jqXHR) => {
			alert(jqXHR.responseJSON.message);
		}
...
const setDetailData = (YMD) => {
    ...
    switch(document.querySelector('.selected').id){
				...
				success: (result) => {
                    ...
                    switch(data.mealServiceDietInfo[0].head[1].RESULT.CODE){
                        case 'INFO-000':
                            ...
                            calHtml += `<tr>
                            <td>중식</td>
                            <td>${meal.DDISH_NM.replace(/<br\/>/ig, '\n')}</td>
                            <td>
        ...
        case 'all_schedule':
        	$.ajax({
				url: `${location.pathname}/my_schedule?YMD=${YMD}`,
				method: 'get',
				dataType: 'json',
				async: false,
				success: (result) => {
					result.data.map(schedule_set => {
                        calHtml += `<tr>
                        <td>일정</td>
                        <td>${schedule_set.schedule.subject} ${Math.abs(new Date(year, month - 1, day) - new Date(schedule_set.schedule.startDate)) / (1000 * 3600 * 24) + 1}일차</td>
                        <td><div><a class='my_schedule' href='#my_schedule_layer' onclick=setMyScheduleLayerData(${schedule_set.schedule.id})>자세히보기</a></div></td>
                        </tr>`
                    });
				},
				error: (jqXHR) => {
