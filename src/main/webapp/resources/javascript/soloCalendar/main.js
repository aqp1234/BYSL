...
const openUpdatePage = () => {
    ...
    openWin = window.open(`${location.pathname}/update/${solo_calendar_id}`, 'childForm', 'width=570, height=400, resizable=no, scrollbars=yes');
}
...
const deleteCalendar = async() => {
    ...
    if(confirm('정말 삭제하시겠습니까?')){
		$.ajax({
			url: `${location.pathname}/${solo_calendar_id}`,
			method: 'delete',
			dataType: 'json',
			async: true,
			success: (result) => {
...
const setCalendarData = async (year, month) => {
    ...
    $.ajax({
		...
		async: true,
		success: (result) => {
			result.data.map(solo_calendar_sets => {
	            let start_date = new Date(solo_calendar_sets.startDate);
	            let end_date = new Date(solo_calendar_sets.endDate);
	            for(let i = 0; i < Math.abs(end_date - start_date) / (1000 * 3600 * 24) + 1; i++){
	                ...
	                if(solo_calendar_id_span.parentNode.childElementCount < 4){
	                    let calHtml = "";
	                    calHtml += `<div>
	                    <a class='solo_calendar' href='#solo_calendar_layer' onclick=setSoloScheduleLayerData(${solo_calendar_sets.id}) style='font-size: 13px; font-weight: bolder; color: #515151; text-decoration: none; margin-top: 10px;'>
	                        ${solo_calendar_sets.subject} ${i + 1}일차
	                    </a>
	                    </div>`;
...
const setSoloScheduleLayerData = async (solo_calendar_id) => {
	$.ajax({
		url: `${location.pathname}/my_calendar_by_id?id=${solo_calendar_id}`,
		method: 'get',
		dataType: 'json',
		async: true,
		success: (result) => {
			const data = result.data;
			document.getElementById('solo_calendar_id').value = data.id;
	        ...
	        document.getElementById('solo_calendar_start_date').innerText = setDateFormat(data.startDate);
	        document.getElementById('solo_calendar_end_date').innerText = setDateFormat(data.endDate);
		},
		error: (jqXHR) => {
			alert(jqXHR.responseJSON.message);
		},
...
const setDetailData = async(YMD) => {
    ...
    const date = String(YMD).substring(6,8);
    deleteDetailData();
    $.ajax({
		url: `${location.pathname}/my_calendar?year=${year}&month=${month}&date=${date}`,
		method: 'get',
		dataType: 'json',
		async: true,
		success: (result) => {
			result.data.map(solo_calendar_set => {
                calHtml += `<tr>
                <td>일정</td>
                <td>${solo_calendar_set.subject} ${Math.abs(new Date(year, month - 1, date) - new Date(solo_calendar_set.startDate)) / (1000 * 3600 * 24) + 1}일차</td>
                <td><div><a class='solo_calendar' href='#solo_calendar_layer' onclick=setSoloScheduleLayerData(${solo_calendar_set.id})>자세히보기</a></div></td>
                </tr>`
            });
            if(calHtml == ''){
